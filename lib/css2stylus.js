    ...
    _parse: function () {
      ...
      var CSSWithoutMedia = this.css
        ...
        .replace(/([^{]+)\{([^}]+)\}/g, function (group, selector, declaration) {
          ...
          if (/,/.test(selector)) {
            console.log('// skip grouped selector');
            console.log(path);
          ...
          // process
            ...
            console.log('// join special chars to prevent from breaking them into parts');
            console.log(selector);

            // split on spaces to get full selector path
            selectors = selector.split(/[\s]+/);
            console.log('// split on spaces to get full selector path');
            ...
            for (i = 0, l = selectors.length; i < l; i++) {
              ...
              console.log('// Added rule to path');
              console.log(path);
              console.log(_sel);
            }
          }
      ...
      jsonfile.writeFile('./tmp/data.json', tree, {spaces: 2}, function (err) {
        console.error(err)
      })


      // // console.log( 'this._generateOutput(tree)' );
      // // console.log( this._generateOutput(tree) );
